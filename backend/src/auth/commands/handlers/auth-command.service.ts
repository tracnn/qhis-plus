...
export class LoginHandler implements ICommandHandler<LoginCommand> {
    ...
    async execute(command: LoginCommand): Promise<any> {
        this.logger.log(`Đăng nhập: ${command.identifier}`);
        try {
            ...
            if (!user) {
                this.logger.warn(`Đăng nhập thất bại: User không tồn tại - ${command.identifier}`);
            ...
            if (!isPasswordValid) {
                this.logger.warn(`Đăng nhập thất bại: Password không đúng - ${command.identifier}`);
                throw new ApiException('INVALID_CREDENTIALS');
            }

            // Kiểm tra user có bị khóa không
            if (user.isLocked) {
                this.logger.warn(`Đăng nhập thất bại: User bị khóa - ${command.identifier}`);
                throw new ApiException('ACCOUNT_LOCKED');
            }

            if (!user.isActive) {
                this.logger.warn(`Đăng nhập thất bại: User chưa kích hoạt - ${command.identifier}`);
            ...
            this.logger.log(`Đăng nhập thành công: ${command.identifier}`);
            return user;
        } catch (error) {
