...
@Entity('XML8_MEDICAL_RECORD_SUMMARY')
export class Xml8MedicalRecordSummary extends BaseEntity {
    @Column({ name: 'XML1_ID' })
    @Index()
    xml1Id: string;
    ...
    @Column({ name: 'NGAY_SINHCON', type: 'varchar', length: 8, nullable: true })
    ngaySinhCon: string;

    @Column({ name: 'NGAY_CONCHET', type: 'varchar', length: 8, nullable: true })
    ngayConChet: string;
    ...
    @Column({ name: 'NGAY_CT', type: 'varchar', length: 8, nullable: true })
    ngayChungTu: string;
