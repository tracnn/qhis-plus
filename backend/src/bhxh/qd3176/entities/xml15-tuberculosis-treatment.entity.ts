...
export class Xml15TuberculosisTreatment extends BaseEntity {
    ...
    @Column({ name: 'NGAYBD_DTRI_LAO', length: 8, nullable: true })
    ngayBatDauDieuTriLao: string;

    @Column({ name: 'PHACDO_DTRI_LAO', type: 'int', nullable: true })
    phacDoDieuTriLao: number;

    @Column({ name: 'NGAYKT_DTRI_LAO', length: 8, nullable: true })
    ...
    @Column({ name: 'NGAYKD_HIV', length: 8, nullable: true })
    ngayKhangDinhHiv: string;

    @Column({ name: 'BDDT_ARV', length: 8, nullable: true })
    batDauDieuTriArv: string;

    @Column({ name: 'NGAY_BAT_DAU_DT_CTX', length: 8, nullable: true })
