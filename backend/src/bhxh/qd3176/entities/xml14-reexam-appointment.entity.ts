...
@Entity('XML14_REEXAM_APPOINTMENT')
export class Xml14ReexamAppointment extends BaseEntity {
    @Column({ name: 'XML1_ID' })
    @Index()
    xml1Id: string;
    ...
    @Column({ name: 'NGAY_HEN_KL', length: 8, nullable: true })
    ngayHenKl: string;
    ...
    @Column({ name: 'NGAY_CT', length: 8, nullable: true })
    ngayCapGiay: string;
