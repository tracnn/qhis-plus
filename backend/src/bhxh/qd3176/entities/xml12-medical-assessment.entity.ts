...
export class Xml12MedicalAssessment extends BaseEntity {
    ...
    @Column({ name: 'NGAY_HOP', length: 8, nullable: true })
    ngayHop: string;

    @Column({ name: 'HO_TEN', length: 255, nullable: true })
    hoTen: string;

    @Column({ name: 'NGAY_SINH', length: 8, nullable: true })
    ngaySinh: string;

    @Column({ name: 'SO_CCCD', type: 'varchar', length: 20, nullable: true })
    soCccd: string;

    @Column({ name: 'NGAY_CAP_CCCD', length: 8, nullable: true })
    ngayCapCccd: string;
    ...
    @Column({ name: 'NGAY_CHUNG_TU', length: 8, nullable: true })
    ngayChungTu: string;

    @Column({ name: 'SO_GIAY_GIOI_THIEU', length: 200, nullable: true })
    soGiayGioiThieu: string;

    @Column({ name: 'NGAY_DE_NGHI', length: 8, nullable: true })
    ngayDeNghi: string;
